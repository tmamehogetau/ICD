...
test("次ラウンド後の決選投票も順不同で、最後の一票だけ結果を確定する", () => {
  ...
  const previousHands = state.players.flatMap(player => player.hand);
  ...
  assert.deepEqual(state.players.map(player => player.hand.length), [5, 5, 5]);
  assert.ok(previousHands.every(cardId => state.adjustmentDiscard.includes(cardId)));
  assert.ok(state.players.every(player => player.hand.every(cardId => !previousHands.includes(cardId))));

  state.round = 4;
  state.stage = "round_results";
  setFinalCandidates(state);
  state.bestCards[0].overpowered = true;
  state.bestCards[0].score = 0;
  continueOnlineRound(state, "u1");
  ...
  assert.equal(state.signage[0].finalAppeal, 4);
