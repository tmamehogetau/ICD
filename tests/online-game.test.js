...
test("幹事は参加中の別プレイヤーへ引き継げる", () => {
  const state = createOnlineGame({ players, rounds: 4, rng });
  transferOnlineHost(state, "u2");
  assert.equal(state.hostId, "u2");
  assert.equal(viewOnlineGame(state, "u2").viewer.isHost, true);
  assert.throws(() => transferOnlineHost(state, "unknown"), /参加者/);
});
