...
test("数値プラスと数値マイナスは指定された符号と0だけを割り振る", () => {
  ...
  addNumericApplication(plus, { id: "a10", text: "数値プラス" }, ["stat:attack", "stat:health"], { deltas: { "stat:attack": 3, "stat:health": 0 } });
  assert.deepEqual([buildDesignDraft(plus).attack, buildDesignDraft(plus).health], [6, 4]);
  ...
  addNumericApplication(minus, { id: "a11", text: "数値マイナス" }, ["stat:attack", "stat:health"], { deltas: { "stat:attack": -3, "stat:health": 0 } });
  assert.deepEqual([buildDesignDraft(minus).attack, buildDesignDraft(minus).health], [0, 4]);
});
