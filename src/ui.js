...
function setup() {
  ...
  root.innerHTML = `${header("今夜の卓を開く", "IZAKAYA / OPEN TABLE")}<section class="setup-sheet paper"><div class="red-note">3〜6名<br>各自の端末で参加</div><div class="online-intro"><b>招待制・アカウント不要</b><span>手札と未公開票は本人だけに届きます。</span></div><label class="wide player-name-field">あなたの名前<input id="player-name" maxlength="16" value="先輩" required></label><div class="table-entry"><form id="create-form"><fieldset><legend>新しい卓を作る</legend><p class="agreement">今夜は全4ラウンド。最終投票の最多得票カードの作者には3点（得票は累計に加算しません）。</p><button class="primary stamp-button">卓を作成する</button></fieldset></form><form id="join-form" class="join-form"><fieldset><legend>卓番号で参加・復帰</legend><label>卓番号<input name="code" maxlength="7" required></label><p class="agreement">同じ名前で入り直すと、元の参加者として復帰します。</p><button class="secondary">卓に参加する</button></fieldset></form></div></section>`;
}
function lobby() {
...
function numericPicker(selected) {
  ...
  return `<section class="placement-panel paper"><p class="eyebrow">NUMERIC PLACEMENT</p><h3>${esc(selected.name)} <small>${esc(countHint)}</small></h3><p>${esc(selected.text)}</p>${selected.id === "a3" ? `<div class="choice-row compact-choice"><label class="radio-card"><input type="radio" name="numeric-delta" value="1" checked><b>＋1</b></label><label class="radio-card"><input type="radio" name="numeric-delta" value="-1"><b>−1</b></label></div>` : ""}<div class="target-grid">${targets.map(target => `<div class="number-target ${selectedIds.has(target.id) ? "selected" : ""}"><button type="button" data-number-target="${target.id}"><b>${esc(target.label)}</b><strong>${target.value}</strong></button>${selectedIds.has(target.id) ? (selected.id === "a10" ? `<label>配分<input aria-label="${esc(target.label)}への振り分け" data-delta-target="${target.id}" type="number" min="0" step="1" value="${pendingDeltas[target.id] ?? 0}"></label>` : selected.id === "a11" ? `<label>配分<input aria-label="${esc(target.label)}への振り分け" data-delta-target="${target.id}" type="number" max="0" step="1" value="${pendingDeltas[target.id] ?? 0}"></label>` : "") : ""}</div>`).join("")}</div><div class="placement-actions"><button type="button" class="secondary" data-action="cancel-adjustment">選び直す</button><button type="button" class="primary" data-action="apply-numeric">この数値配置を反映</button></div></section>`;
}
...
function vote(final = false) {
  ...
  const ownBallot = final ? meeting.private?.finalBallot : meeting.private?.roundBallot;
  const canCancel = final ? meeting.private?.canCancelFinalVote : meeting.private?.canCancelRoundVote;
  if (!mine()) {
    const submitted = final ? meeting.submission?.finalVotesSubmitted || 0 : meeting.submission?.votesSubmitted || 0;
    const players = meeting.submission?.playerCount || 0;
    if (!ownBallot) return waiting(final ? "決選投票中" : "ヤバいカード投票中", `ほかの参加者の投票待ち（${submitted} / ${players}）。`);
    const targetName = id => esc(cards.find(card => card.id === id)?.playerName || "なし");
    return root.innerHTML = `${header(final ? "決選票を提出しました" : "ヤバい票を提出しました", "VOTE / SUBMITTED")}<section class="submitted-sheet paper"><div class="submitted-stamp">投票済み</div><h2>ほかの参加者の投票を待っています</h2><p class="agreement">現在の提出数：${submitted} / ${players}</p><div class="ballot-summary"><span>2点票：${targetName(ownBallot.two)}</span><span>1点票：${targetName(ownBallot.one)}</span></div>${canCancel ? `<div class="center-action"><button class="secondary" data-action="${final ? "cancel-final-vote" : "cancel-round-vote"}">投票を取り消して選び直す</button></div>` : ""}</section>`;
  }
...
function finalOverview() { root.innerHTML = `${header("今夜の看板カード決選", "FINAL AGENDA / SHORTLIST")}<div class="notice paper"><b>再プレゼンは15秒</b><span>最多得票カードの作者に3点。決選の得票自体は累計に加算しません。</span></div><div class="design-grid finalists">${meeting.bestCards.map(card => designCard(card, true)).join("")}</div><div class="center-action">${host() ? '<button class="primary stamp-button" data-action="begin-final-voting">決選投票へ</button>' : '<p class="agreement">幹事が決選投票を開始するのを待っています。</p>'}</div>`; }
function finalResults() { const top = meeting.rankings[0]?.total; root.innerHTML = `${header("お開き・最終発表", "MEETING CLOSED / APPROVED")}<section class="hero-result paper"><div class="giant-stamp">新弾<br>看板</div><p>NEW PACK SIGNATURE CARD</p><h2>${meeting.signage.map(card => esc(card.name)).join(" / ")}</h2><span>担当：${meeting.signage.map(card => esc(card.playerName)).join(" / ")}</span></section><section class="winner-showcase"><h3>今夜の最優秀カード</h3><div class="design-grid finalists">${meeting.signage.map(card => designCard(card, true, true)).join("")}</div></section><section class="final-ranking paper"><h3>最終順位</h3>${meeting.rankings.map((item, index) => `<div class="rank-row ${item.total === top ? "champion" : ""}"><span class="rank">${String(index + 1).padStart(2, "0")}</span><b>${esc(item.name)}</b><strong>${item.total}点</strong>${item.total === top ? "<em>最優秀デザイナー</em>" : ""}</div>`).join("")}</section><div class="center-action"><button class="secondary" data-action="leave">この端末の参加情報を消す</button></div>`; }
function render() {
  if (!meeting) setup(); else if (meeting.stage === "lobby") lobby(); else if (meeting.stage === "round_reveal") reveal(); else if (["pass_build", "pass_vote", "pass_final_vote"].includes(meeting.stage)) pass(meeting.stage); else if (meeting.stage === "build") { if (meeting.private?.phase === "submitted") submittedDesign(); else build(); } else if (meeting.stage === "presentations") presentations(); else if (meeting.stage === "vote") vote(); else if (meeting.stage === "round_results") results(); else if (meeting.stage === "final_overview") finalOverview(); else if (meeting.stage === "final_vote") vote(true); else if (meeting.stage === "final_results") finalResults();
...
root.addEventListener("click", async event => {
  ...
  try { const action = button.dataset.action;
    ...
    if (action === "cancel-round-vote") { await command("cancelRoundVote"); return; }
    if (action === "cancel-final-vote") { await command("cancelFinalVote"); return; }
...
root.addEventListener("click", event => {
  ...
  try {
    ...
    if (crestEffectBlock && selectedAdjustment() && ["効果", "条件"].includes(selectedAdjustment().category)) {
      ...
      if (card.choices?.length && !pendingChoice) throw new Error("能力を1つ選んでください。");
      const params = pendingChoice ? { choice: pendingChoice } : {};
      if (card.category === "効果") addEffectApplication(editor, card, 0, params, "crest_effect"); else addConditionApplication(editor, card, 0, params, "crest_effect");
      selectedAdjustmentId = null; pendingTargets = []; pendingDeltas = {}; pendingChoice = ""; render(); return;
    }
...
function planCard(draft, selected) {
  ...
  const canPlace = Boolean(selected && ["効果", "条件"].includes(selected.category) && (!selected.choices?.length || pendingChoice));
