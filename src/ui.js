...
root.addEventListener("submit", async event => {
  ...
  try {
    ...
    if (event.target.id === "design-form") { const draft = buildDesignDraft(editor); await command("submitDesign", { name: draftName.trim() || meeting.base.name, cost: draft.cost, attack: draft.attack, health: draft.health, effect: draft.effect, intent: "", adjustmentIds: editor.applications.map(item => item.adjustmentId) }); }
